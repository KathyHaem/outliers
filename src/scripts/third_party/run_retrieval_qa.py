...
from scripts.third_party.processors.lareqa import (
...
ALL_MODELS = sum(
  (tuple(conf.keys()) for conf in (BERT_PRETRAINED_CONFIG_ARCHIVE_MAP,)),
  (),
)
...
def train(args, train_dataset, model, tokenizer):
  ...
  for _ in train_iterator:
    epoch_iterator = tqdm(train_dataloader, desc="Iteration", miniters=1000, disable=args.local_rank not in [-1, 0])
