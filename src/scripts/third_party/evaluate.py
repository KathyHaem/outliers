...
from scripts.third_party.evaluate_squad import evaluate as squad_eval
from scripts.third_party.evaluate_mlqa import evaluate as mlqa_eval


def read_tag(file):
    labels = []  # list of all examples
    example = []  # list of tags in the example
...
def evaluate_one_task(prediction_file, label_file, task, language=None):
    """Evaluate the classification tasks by accuracy.
    Args:
      prediction_file (string): path to the prediction tsv file.
      label_file (string): path to the ground truth tsv file.
    Return:
    ...
    if task not in ['bucc2018', 'mlqa', 'tydiqa', 'xquad']:
        assert len(predictions) == len(labels), 'Number of examples in {} and {} not matched in {} task'.format(
...
def evaluate(prediction_folder, label_folder, verbose=False):
    ...
    print(f"Label tasks: {label_tasks}")
    ...
    for task, langs in TASK2LANGS.items():
        if task in prediction_tasks and task in label_tasks:
            ...
            for lg in langs:
                ...
                if task == "bucc2018":
                    #prediction_file = os.path.join(prediction_folder, task, f"xlm-roberta-base-{lg}" ,f"candidates_7_dev.{suffix}")
                    prediction_file = os.path.join(prediction_folder, task, f"test-{lg}.{suffix}")
                    print(f"Prediction file: {prediction_file}")
                    label_file = os.path.join(label_folder, task, f"{lg}-en.dev.gold")
