...
def heatmaps(tgt_embs, eng_embs, target_lang):
    TGT = target_lang
    dir_tgt = f'plots_layer_{args.layer}/{TGT}/{args.model}/{TGT}.png'
    dir_eng = f'plots_layer_{args.layer}/{TGT}/{args.model}/eng.png'
    ...
    plt.title(f"{TGT} representations")
    plt.ylabel("# sentence")
    ...
    # plt.show()
    print(f"Finished {TGT} heatmaps.")


# produce visualizations of language level mean embeddings (for target and english)
def mean_vec(tgt_embs, eng_embs, target_lang):
    TGT = target_lang
    dir_tgt = f'plots_layer_{args.layer}/{TGT}/{args.model}/mean_{TGT}.png'
    dir_eng = f'plots_layer_{args.layer}/{TGT}/{args.model}/mean_eng.png'
    os.makedirs(os.path.dirname(dir_tgt), exist_ok=True)
    os.makedirs(os.path.dirname(dir_eng), exist_ok=True)
    ...
    plt.title(f"{TGT} mean representation")
    plt.xlabel("dimension")
    plt.savefig(dir_tgt, dpi=300, bbox_inches="tight")
    # plt.show()
    ...
    plt.savefig(dir_eng, dpi=300, bbox_inches="tight")
    # plt.show()
    print(f"Finished {TGT} mean vecs.")
...
def mean_english():
    ...
    mean_vec = torch.divide(mean_sum_vec, 36)
...
def plot_outliers(tgt_embs, eng_embs, target_lang):
    TGT = target_lang
    dir_tgt = f'dim_plots_layer_{args.layer}/{TGT}/{args.model}/{TGT}_{args.dim}.png'
    dir_tgt_en = f'dim_plots_layer_{args.layer}/{TGT}/{args.model}/eng_{TGT}_{args.dim}.png'
    os.makedirs(os.path.dirname(dir_tgt), exist_ok=True)
    ...
    plt.plot(tgt_embs.T[args.dim][:-1], marker='x', linestyle='None', markersize=2, color='xkcd:soft green', label=TGT)
    ...
    plt.title(f"{lang_dict_3_2[TGT]}-en dimension {args.dim}")
    plt.xlabel("#sentence")
...
def main():
    if args.job == "heatmaps":
        ...
        for lang in langs:
            ...
            eng_emb = torch.load(f'embs_layer_{args.layer}/{args.model}/{TGT}/eng.pt')
            heatmaps(target_emb, eng_emb, TGT)
        ...
        for lang in langs:
            ...
            target_emb = torch.load(f'embs_layer_{args.layer}/{args.model}/{TGT}/{TGT}.pt')
            eng_emb = torch.load(f'embs_layer_{args.layer}/{args.model}/{TGT}/eng.pt')
            mean_vec(target_emb, eng_emb, TGT)

    elif args.job == "outlier":
        print("Generating outlier visualization.")
        if args.language != "":
            TGT = lang_dict[args.language]
            target_emb = torch.load(f'embs_layer_{args.layer}/{args.model}/{TGT}/{TGT}.pt')
            eng_emb = torch.load(f'embs_layer_{args.layer}/{args.model}/{TGT}/eng.pt')
            plot_outliers(target_emb, eng_emb, TGT)
        else:
            for lang in langs:
                print(f"Current language: {lang}")
                TGT = lang
