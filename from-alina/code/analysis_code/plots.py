...
def heatmaps(tgt_embs, eng_embs, tgt_lang):
    fig_dir = f'../plots/{args.model}/{args.layer}/{tgt_lang}/'
...
def mean_english():
    fig_dir = f'../plots/{args.model}/{args.layer}/'
    os.makedirs(os.path.dirname(fig_dir), exist_ok=True)
    mean_sum_vec = None
    for lang in langs:
        print(f"Processing parallel English of: {lang}")
        eng_emb = torch.load(f'../embs/{args.model}/{args.layer}/{lang}/eng.pt')
    ...
    plt.plot(mean_vec)
    plt.fill_between(x, mean - std_dev * 3, mean + std_dev * 3, alpha=0.5, facecolor=clrs[7])
...
def plot_outliers(tgt_embs, eng_embs, tgt_lang):
    dir_tgt = f'../plots/{args.model}/{args.layer}/{tgt_lang}/{tgt_lang}_dim_{args.dim}.png'
    dir_tgt_en = f'../plots/{args.model}/{args.layer}/{tgt_lang}/eng_{tgt_lang}_dim_{args.dim}.png'
...
def main():
    if args.job == "heatmaps":
        ...
        for lang in langs:
            ...
            target_emb = torch.load(f'../embs/{args.model}/{args.layer}/{lang}/{lang}.pt')
            eng_emb = torch.load(f'../embs/{args.model}/{args.layer}/{lang}/eng.pt')
